...
class SpringSecurityOAuthController {
        ...
        if (request.post) {
            if (!springSecurityService.loggedIn) {
                ...
                boolean created = command.validate() && User.withTransaction { status ->
                    User user = new User(phone: command.username, password: command.password1, enabled: true)
                    user.addToOAuthIDs(provider: oAuthToken.providerName, accessToken: oAuthToken.socialId, user: user)
                    ...
                    new UserRole(user: user, role:  Role.findByAuthority(AuthKeys.USER)).save(flush: true)
    ...
    def unLinkAccount() {
        ...
        if(!user){
            renderError 500, "User not found"
            return
        }
        AuthenticationToken.findAllByUsername(user.username)?.each {
            it.delete(flush: true)
        }
        OAuthID.findAllByUser(user)?.each {
            it.delete(flush: true)
        }
...
*/
    ...
    protected void authenticateAndRedirect(OAuthToken oAuthToken, redirectUrl) {
        ...
        if(uri){
            if (redirectUrl instanceof Map)
                redirectUrl.put('uri', uri)
            else
                redirectUrl = uri
        }
        redirect(redirectUrl instanceof Map ? redirectUrl : [uri: redirectUrl])
...
class OAuthCreateAccountCommand {
    ...
    static constraints = {
        businessId blank: true, nullable: true
        masterRole blank: true, nullable: true
        userRole blank: true, nullable: true
